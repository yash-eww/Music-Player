...
const Page = ({ params }: { params: { id: string } }) => {
  const { setSong, song, setPlaySong } = useAudio();

  useEffect(() => {
    const fetchAlbum = async () => {
      try {
        const res = await fetch(`https://saavn.me/albums?id=${params.id}`);
    ...
    <div>
      <DashBoard>
        <div className="p-10 pt-32 bg-[#16151A] text-white h-screen">
          <h1 className="text-[32px]">{song?.name}</h1>
                ...
                <div className="flex p-5 justify-between w-full h-fit">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[#bdbac2]">{x.primaryArtists}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button className="bg-[#192B26] w-8 h-8 rounded-lg">
                      <AddRoundedIcon
                        htmlColor="#249E66"
                        sx={{
                          width: "20px",
